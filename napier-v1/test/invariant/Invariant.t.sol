...
contract InvariantTest is CompleteFixture {
    ...
    function setUp() public virtual override {
        ...
        excludeSender(address(MockAdapter(address(adapter)).lendingProtocol()));
    }
    ...
    function invariant_Solvency_1() public useCurrentTimestamp {
        ...
        console2.log("Current adapter scale :>>", adapter.scale());

        // Burn all PT and YT
        for (uint256 i = 0; i != recipients.length; i++) {
            uint256 bal = tranche.balanceOf(recipients[i]);
            vm.startPrank(recipients[i]);
            console2.log("Balance before `redeem` :>>", target.balanceOf(address(tranche)));
            tranche.redeem({principalAmount: bal, from: recipients[i], to: address(this)});
            console2.log("Balance after `redeem`:>>", target.balanceOf(address(tranche)));
            if (tranche.lscales(recipients[i]) != 0) tranche.collect();
            vm.stopPrank();

            assertEq(tranche.unclaimedYields(recipients[i]), 0, "Unclaimed yield should be 0");
        }
        assertEq(tranche.totalSupply(), 0, "All PTs should be burned");
        assertEq(yt.totalSupply(), 0, "All YTs should be burned");
        assertGe(target.balanceOf(address(tranche)), 0, Properties.T_01);
    }
    ...
    function invariant_Solvency_2() public useCurrentTimestamp {
        ...
        for (uint256 i = 0; i != recipients.length; i++) {
            ...
            if (pBal >= yBal) {
                ...
                if (yBal > 0) {
                    console2.log("Balance before `redeemWithYT`:>>", target.balanceOf(address(tranche)));

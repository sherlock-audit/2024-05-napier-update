...
contract TestMorphoTranche is BaseTestTranche, MorphoFixture {
    ...
    function testIssue_Ok() public override {
        ...
        uint256 maxscale = Math.max(cscale, tranche.getGlobalScales().maxscale);
        ...
        assertEq(tranche.lscales(user), Math.max(cscale, maxscale), "lscale should be updated to max scale");

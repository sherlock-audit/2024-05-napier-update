...
contract TestTranche is BaseTestTranche {
    ...
    // computePrincipalTokenRedeemed(computeSharesRedeemed(x)) <=~ x
    function testRT_ComputeShares_ComputePrincipal(
        ...
        // Need more principal tokens
        assertApproxLeAbs(principal_, principalAmount, 3, "prop/principal->shares->principal");
    }

    // computeSharesRedeemed(computePrincipalTokenRedeemed(s)) <=~ s
    function testRT_ComputePrincipal_ComputeShares(uint128 mscale, uint128 maxscale, uint128 shares) public virtual {
        ...
        // Need more shares
        assertApproxGeAbs(shares_, shares, 3, "prop/shares->principal->shares");
    }
    ...
    function testRedeemWithYt_WithAllowOther() public {
        ...
        deal(address(underlying), address(this), 100e6, true);
        tranche.issue(address(bob), 100e6);
        ...
        // 4. zero unclaimed and pending yield drained by bob
        assertEq(0, underlying.balanceOf(bob));
        vm.prank(bob);
        tranche.redeemWithYT(address(this), bob, 1);
        assertEq(underlying.balanceOf(bob), 0, "bob should not receive any underlying");
    }

...
contract RouterRemoveLiquidityFuzzETHTest is RouterSwapFuzzTest {
        ...
        setUpRandomReserves(input.ptsToBasePool)
    {
        ...
        amounts[1] = 0;
        amounts[2] = 0;
        data[2] = abi.encodeWithSignature("sweepTokens(address[],uint256[],address)", tokens, amounts, receiver);
...
contract RouterRemoveLiquidityOneUnderlyingFuzzTest is RouterSwapFuzzTest {
    ...
    function testFuzz_removeLiquidityOneUnderlying(
        ...
        data[1] = abi.encodeWithSignature("sweepToken(address,uint256,address)", pts[index], 0, receiver);
        data[2] = abi.encodeWithSignature("sweepToken(address,uint256,address)", tricrypto, 0, receiver);
        bytes[] memory ret = Multicallable(address(router)).multicall(data);
...
contract RouterRemoveLiquidityOneUnderlyingETHFuzzTest is RouterSwapFuzzTest {
    ...
    function testFuzz_removeLiquidityOneUnderlyingETH(
        ...
        // Unwrap WETH, and send ETH to receiver
        data[1] = abi.encodeWithSignature("unwrapWETH9(uint256,address)", 0, receiver);
        data[2] = abi.encodeWithSignature("sweepToken(address,uint256,address)", pts[index], 0, receiver);
        data[3] = abi.encodeWithSignature("sweepToken(address,uint256,address)", tricrypto, 0, receiver);
        bytes[] memory ret = Multicallable(address(router)).multicall(data);
        uint256 underlyingOut = abi.decode(ret[0], (uint256));

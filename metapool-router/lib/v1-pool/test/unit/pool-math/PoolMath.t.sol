...
contract PoolMathDifferentialTest is Base {
    ...
    uint256 constant INITIAL_VIRTUAL_PRICE = 1e18;
    ...
    modifier boundPoolStateInput(PoolState memory state) {
        state.virtualPrice = bound(state.virtualPrice, INITIAL_VIRTUAL_PRICE, 1.2e18);
        ...
        boundPoolStateInput(state)
    {
        ...
        exactBaseLptIn = bound(exactBaseLptIn, 0, state.totalBaseLptTimesN * WAD / (N_COINS * state.virtualPrice));
        try ref.swapExactPtForSy(state.toMarket(), exactBaseLptIn * N_COINS * state.virtualPrice / WAD) {
            vm.assume(false);
    ...
    function testFuzz_RevertIf_EffectiveExchangeRateBelowOne(PoolState memory state, uint256 exactBaseLptOut)
        ...
        boundPoolStateInput(state)
    {
        // In some edge cases, PRBMath::ln(proportion=0) may revert with PRBMath_SD59x18_Log_InputTooSmall,
        // before computing the exchange rate. In this case, the error is not PoolExchangeRateBelowOne.
        // This error can happen when the `exactBaseLptOut` is too large.
        exactBaseLptOut =
            bound(exactBaseLptOut, 0, state.totalBaseLptTimesN * WAD / (N_COINS * state.virtualPrice) * 90 / 100);
        try ref.swapSyForExactPt(state.toMarket(), exactBaseLptOut * N_COINS * state.virtualPrice / WAD) {
            vm.assume(false);
        } catch (bytes memory reason) {
        ...
        boundPoolStateInput(state)
    {
        ...
        if (s) {
            int256 rateAnchor = abi.decode(ret, (int256));
            assertEq(rateAnchor, 1e18, "rate anchor should be 1e18");
        } else {
            // Reverting without any error message is expected
        }
    }
        ...
        boundPoolStateInput(state)
    {
        ...
        (bool s1, bytes memory ret) = address(ref).staticcall(
            abi.encodeCall(
                ref.swapExactPtForSy, (state.toMarket(), exactBaseLptIn * N_COINS * state.virtualPrice / WAD)
            )
        );
        vm.assume(s1);
    ...
    function testFuzz_swapUnderlyingForExactBaseLpt(PoolState memory state, uint256 exactBaseLptOut)
        ...
        boundPoolStateInput(state)
    {
        state.virtualPrice = INITIAL_VIRTUAL_PRICE; // For simplicity, we set the virtual price to 1e18
        state.totalBaseLptTimesN = bound(state.totalBaseLptTimesN, 1e18, 1e9 * 1e18);
        state.totalUnderlying18 = bound(state.totalUnderlying18, 1e18, 1e9 * 1e18);
        // Note Lower bound is 1e-3 * 1e18
        exactBaseLptOut = bound(exactBaseLptOut, 1e15, state.totalBaseLptTimesN * WAD / (N_COINS * state.virtualPrice));

        (bool s1, bytes memory ret) = address(ref).staticcall(
            abi.encodeCall(
                ref.swapSyForExactPt, (state.toMarket(), exactBaseLptOut * (N_COINS * state.virtualPrice) / WAD)
            )
        );
        vm.assume(s1);
    ...
    function testFuzz_RevertIf_PoolExchangeRateBelowZero_swapUnderlyingForExactBaseLpt(
        ...
        exactBaseLptOut = bound(exactBaseLptOut, 0, state.totalBaseLptTimesN * WAD / (N_COINS * state.virtualPrice) - 1);
        (bool s1, bytes memory ret) = address(ref).staticcall(
            abi.encodeCall(
                ref.swapSyForExactPt, (state.toMarket(), exactBaseLptOut * N_COINS * state.virtualPrice / WAD)
            )
        ...
        // _getRateAnchor may revert with log input zero
        if (bytes4(ret2) == bytes4(keccak256("PRBMath_SD59x18_Log_InputTooSmall(int256)"))) return;

...
contract RouterAddLiquidityOneUnderlyingKeepYtUnitTest is RouterAddLiquidityTest {
    IQuoter quoter;
    ...
    function test_RevertIf_Reentrant() public override {
        ...
        _expectRevertIf_Reentrant(
            address(0xbad),
            abi.encodeWithSelector(
                router.addLiquidityOneUnderlyingKeepYt.selector,
                address(pool),
                1,
                1000,
                0,
        ...
        vm.prank(address(0xbad)); // receiver of the callback
        pool.swapPtForUnderlying(0, 1000, address(0xbad), abi.encode(NapierPool.swapPtForUnderlying.selector, pts[0]));
    }

    function test_RevertIf_MaturityPassed() public override whenMaturityPassed {
        // Tranche.issue function will revert if maturity has passed.
        ...
        router.addLiquidityOneUnderlyingKeepYt({
            ...
            recipient: address(this),
            deadline: block.timestamp,
            underlyingDeposit: 15 * ONE_UNDERLYING / 10
        });
    }

    function test_addLiquidityOneUnderlyingKeepYtWithApprox() public virtual whenMaturityNotPassed {
        pool.skim(); // reserves in pool may be different from the actual balances. skim to make them equal to the actual balances
        uint256 underlyingsToAdd = 10 * ONE_UNDERLYING;
        /// Execute ///
        // 1. Approximate the amount of underlyingDeposit to be swapped out to add liquidity from `underlyingsToAdd` amount of underlying token only
        deal(address(underlying), address(this), underlyingsToAdd, true);
        (, uint256 underlyingDeposit) = quoter.quoteAddLiquidityOneUnderlyingKeepYt(pool, 2, underlyingsToAdd);

        uint256 _before = gasleft();
        uint256 liquidity = router.addLiquidityOneUnderlyingKeepYt(
            address(pool), 2, underlyingsToAdd, 0, address(this), block.timestamp, underlyingDeposit
        );
        console2.log("gas usage: ", _before - gasleft());
    ...
    function test_addLiquidityOneUnderlyingKeepYt() public virtual whenMaturityNotPassed {
        uint256 liquidity = router.addLiquidityOneUnderlyingKeepYt({
            pool: address(pool),
            index: 2,
            underlyingIn: 100 * ONE_UNDERLYING,
            ...
            recipient: address(this),
            deadline: block.timestamp,
            underlyingDeposit: 10 * ONE_UNDERLYING
        });
        assertEq(pool.balanceOf(address(this)), liquidity, "didn't receive enough pool lptoken");
        assertNoFundLeftInPoolSwapRouter();
    }
    ...
    function test_RevertIf_DeadlinePassed() public virtual override {
        ...
        router.addLiquidityOneUnderlyingKeepYt({
            ...
            recipient: address(this),
            deadline: block.timestamp - 1,
            underlyingDeposit: ONE_UNDERLYING
        });
    }
    ...
    function test_RevertIf_InsufficientLpOut() public virtual override {
        ...
        router.addLiquidityOneUnderlyingKeepYt({
            ...
            recipient: address(this),
            deadline: block.timestamp,
            underlyingDeposit: 2 * ONE_UNDERLYING
        });
    }

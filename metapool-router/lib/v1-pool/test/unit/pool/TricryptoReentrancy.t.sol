...
contract TricryptoReentrancyTest is PoolSwapBaseTest {
    ...
    function tricryptoSwapCallback() external {
        ...
        if (_tempPtForUnderlying) {
            ...
            // Actually `get_virtual_price` will revert before `calc_token_amount`, `add_liquidity` or `remove_liquidity_one_coin` are called.
            // Ensure that Tricrypto `get_virtual_price` is called
            vm.expectCall(address(tricrypto), abi.encodeWithSignature("get_virtual_price()"));

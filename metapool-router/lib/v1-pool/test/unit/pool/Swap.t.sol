...
contract PoolSwapPtForUnderlyingUnitTest is PoolSwapBaseUnitTest {
    ...
    function test_swapPtForUnderlying() public whenMaturityNotPassed {
        ...
        // sanity check
        uint256 protocolFee = SwapEventsLib.getLastSwapEvent(pool).protocolFee;
...
contract PoolSwapBaseLpTokenForUnderlyingUnitTest is PoolSwapBaseLptUnitTest {
    ...
    function test_swap() public whenMaturityNotPassed {
        ...
        // assert
        uint256 protocolFee = SwapEventsLib.getLastMetaSwapEvent(pool).protocolFee;
        assertGt(protocolFee, 0, "fee should be accumulated");
        // Pool is initialised with 1/2 proportion of baseLpt
...
contract PoolSwapUnderlyingForBaseLpTokenUnitTest is PoolSwapBaseLptUnitTest {
    function test_swap2() public whenMaturityNotPassed {
        pool.skim(); // Ensure no excess token in pool (fee, etc)
        assertEq(underlying.balanceOf(feeRecipient), 0, "pre-condition: underlying balance should be 0");
        ...
        // balance before swap
        uint256 senderBalance0 = underlying.balanceOf(address(this));
        uint256 poolBalance0 = underlying.balanceOf(address(pool));
        console2.log("Balance before swap: sender=%d, pool=%d", senderBalance0, poolBalance0);
        console2.log("Reserve before swap:", pool.totalUnderlying());

        // swap
        uint256 underlyingIn = pool.swapUnderlyingForExactBaseLpToken(10 * 1e18, recipient);

        // balance after swap
        uint256 senderBalance1 = underlying.balanceOf(address(this));
        uint256 poolBalance1 = underlying.balanceOf(address(pool));
        console2.log("Balance after swap: sender=%d, pool=%d", senderBalance1, poolBalance1);
        console2.log("Reserve after swap:", pool.totalUnderlying());

        // The amount of underlying token transferred to pool equals `underlyingIn`, and `swapFee` is not included.
        assertEq(poolBalance1 - poolBalance0, underlyingIn);

        assertGt(
            underlying.balanceOf(address(pool)), pool.totalUnderlying(), "pool balance should be gt underlying reserve"
        );
        pool.skim(); // Collect swap fee
        assertGt(underlying.balanceOf(feeRecipient), 0, "post-condition: underlying balance should be gt 0");
        console2.log("Fee Recipient collected fee:>>", underlying.balanceOf(feeRecipient));
    }

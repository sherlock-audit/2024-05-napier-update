...
contract PoolAddLiquidityUnitTest is PoolAddLiquidityBaseUnitTest {
    ...
    function test_WhenZeroTotalSupply() public override whenMaturityNotPassed whenZeroTotalSupply {
        ...
        // mint approximately 2970 baseLpt
        {
            // issue pts
            deal(address(underlying), address(this), underlyingIn + N_COINS * underlyingIn, false);
            uint256[N_COINS] memory ptsIssued;
            for (uint256 i = 0; i < N_COINS; i++) {
                _approve(underlying, address(this), address(pts[i]), type(uint256).max);
                ptsIssued[i] = pts[i].issue(address(this), underlyingIn);
            }
            _approvePts(address(this), address(tricrypto), type(uint256).max);
            baseLptIn = tricrypto.add_liquidity(ptsIssued, 0, false, alice);
        }
    ...
    function test_RevertWhen_ProportionTooHigh() public {
        ...
        uint256 underlyingIn = 3000 * ONE_UNDERLYING;
        // Need to mock get_virtual_price to avoid revert
        vm.mockCall(address(tricrypto), abi.encodeWithSelector(tricrypto.get_virtual_price.selector), abi.encode(1e18));

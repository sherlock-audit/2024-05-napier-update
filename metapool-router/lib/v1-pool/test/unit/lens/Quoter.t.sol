...
contract QuoteTest is PoolSwapFuzzTest {
    ...
    function test_quoteAddLiquidityOneUnderlyingKeepYt(
        RandomBasePoolReservesFuzzInput memory input,
        uint256 underlyingIn
    ...
    function test_quoteUnderlyingForYt(SwapFuzzInput memory input, uint256 ytDesired, uint256 cscale)
        ...
        setUpRandomBasePoolReserves(input.ptsToBasePool)
    {
        ytDesired = bound(ytDesired, ONE_UNDERLYING / 100, yts[input.index].totalSupply() * 30 / 100);
        ...
        // mock scale change
        {
            uint256 scale = adapters[input.index].scale();
            cscale = bound(cscale, scale * 90 / 100, scale * 180 / 100);
            mockCallAdapterScale(input.index, cscale);
        }
        // Simulate swap
        vm.recordLogs();
        uint256 snapshot = vm.snapshot();
        uint256 actualUnderlyingIn;
        {
            ...
            bytes memory data = abi.encodeWithSignature(
                "swapUnderlyingForYt(address,uint256,uint256,uint256,address,uint256)",
                address(pool),
                input.index,
                ytDesired,
                type(uint128).max,
            ...
            actualUnderlyingIn = abi.decode(returndata, (uint256));
        }
        // Execute
        vm.revertTo(snapshot); // revert to state before swap and check quote result
        (uint256 underlyingIn, uint256 uDeposit,) = quoter.quoteUnderlyingForYt(pool, input.index, ytDesired);

        uint256 actualDeposited = actualUnderlyingIn + uint256(SwapEventsLib.getLastSwapEvent(pool).netUnderlying);
        // Assert
        assertApproxEqRel(underlyingIn, actualUnderlyingIn, 0.000001 * 1e18, "quote underlying for yt");
        assertApproxEqRel(uDeposit, actualDeposited, 0.05 * 1e18, "quote underlying deposit");
    ...
    {
        uint256 index = input.index;
        ytDesired = bound(ytDesired, ONE_UNDERLYING / 10, pool.totalUnderlying() * 80 / 100);
        // Pre-condition
        vm.warp(input.timestamp);
        {
            // mock scale change
        ...
        if (!success) {
            ...
            vm.assume(
                !(
                    bytes4(returndata) == Errors.ApproxFailWithHint.selector
                        || bytes4(returndata) == Errors.ApproxFail.selector
                )
            );
    ...
    function test_approxUnderlyingToAddLiquidityOneUnderlyingKeepYt(
        SwapFuzzInput memory input,
        uint256 underlyingsToAdd
    ) public boundSwapFuzzInput(input) setUpRandomReserves(input.ptsToBasePool) {
        uint256 index = input.index;
        // Ensure that no tokens are left in the pool after setup operation
        pool.skim(); // reserves in pool may be different from the actual balances. skim to make them equal to the actual balances
        vm.warp(input.timestamp);
        underlyingsToAdd = bound(underlyingsToAdd, ONE_UNDERLYING, pool.totalUnderlying() * 50 / 100);

        deal(address(underlying), alice, underlyingsToAdd, false);
        // deal(address(tricrypto), alice, 0, false);
        _approvePts(alice, address(tricrypto), type(uint256).max);
        _approve(tricrypto, alice, address(pool), type(uint256).max);
        _approve(underlying, alice, address(pts[index]), type(uint256).max);

        /// Execute ///
        uint256 approxUnderlying =
            quoter.approxUnderlyingToAddLiquidityOneUnderlyingKeepYt(pool, index, underlyingsToAdd);

        // 1. Issue principal token with the amount of `approxUnderlying`
        vm.startPrank(alice);
        uint256 pyIssued = pts[index].issue({to: alice, underlyingAmount: approxUnderlying});

        // 2. Deposit the issued principal token to Tricrypto pool
        uint256[3] memory amounts;
        amounts[index] = pyIssued;
        uint256 baseLptIn = tricrypto.add_liquidity(amounts, 0);

        // 3. Add liquidity to Napier pool with the remaining underlying token
        uint256 remainingUnderlying = underlyingsToAdd - approxUnderlying;
        ...
        assertReserveBalanceMatch();
        assertPoolReserveRatio(
            [baseLptIn, remainingUnderlying],
            [uint256(pool.totalBaseLpt()), pool.totalUnderlying()],
            0.001 * 1e18 // large error margin for this case
        );
        // Note: In theory, Alice shouldn't have any remaining underlying token and baseLpt token after adding liquidity.
        assertLe(
            underlying.balanceOf(alice),
            remainingUnderlying / 10_000,
            "should use almost all baseLpt token to add liquidity"
        );
        assertLe(tricrypto.balanceOf(alice), baseLptIn / 10_000, "should use almost all baseLpt token to add liquidity");
    }
        ...
        setUpRandomReserves(input.ptsToBasePool)
    {
        ...
        console2.log("underlying.balanceOf(address(pool)); :>>", underlying.balanceOf(address(pool)));
        console2.log("pool.totalUnderlying() :>>", pool.totalUnderlying());

...
contract TrancheRouterTest_ERC20 is Base {
    ...
    function testIssue_RevertIfMatured() public virtual {
        ...
        trancheRouter.issue(address(adapters[0]), maturity, 1 ether, receiver, 0);
        vm.stopPrank();
    }
    ...
    function testIssue_RevertIfNotWETHUnderlying() public virtual {
        startHoax(caller);
        vm.expectRevert("ERC20: insufficient allowance");
        trancheRouter.issue{value: 1 ether}(address(adapters[0]), maturity, 1 ether, receiver, 0);
        vm.stopPrank();
    }

    function testIssue_Ok() public virtual {
        address underlying = ITranche(pts[0]).underlying();
        uint256 minIssue = 0.9 ether;

        _approve(IERC20(underlying), caller, address(trancheRouter), 1 ether);
        vm.startPrank(caller);
        uint256 issuedAmount = trancheRouter.issue(address(adapters[0]), maturity, 1 ether, receiver, minIssue);
        assertGe(issuedAmount, minIssue, "issuedAmount should be greater than minIssue");
        vm.stopPrank();
    }

    function testIssue_RevertIfInsufficientPyIssue() public virtual {
        address underlying = ITranche(pts[0]).underlying();

        _approve(IERC20(underlying), caller, address(trancheRouter), 1 ether);
        startHoax(caller);
        vm.expectRevert(Errors.RouterInsufficientPyIssue.selector);
        trancheRouter.issue{value: 1 ether}(address(adapters[0]), maturity, 1 ether, receiver, 1 ether + 1);
        vm.stopPrank();
    }
    ...
    function testRedeemWithYT_RevertIfInsufficientYieldTokenBalance() public virtual {
        ...
        _approve(IERC20(underlying), caller, address(trancheRouter), 1 ether);
        vm.prank(caller);
        uint256 issuedAmount = trancheRouter.issue(address(adapters[0]), maturity, 1 ether, caller, 0);
...
contract TrancheRouterTest_ETH is TrancheRouterTest_ERC20 {
    ...
    function testIssue_RevertIfInsufficientAllowance() public override {
        vm.startPrank(callerWithETH);
        vm.expectRevert("ERC20: insufficient allowance");
        trancheRouter.issue{value: 0.5 ether}(address(adapters[0]), maturity, 1 ether, receiver, 0);
        vm.stopPrank();
    }
    ...
    function testIssue_Ok() public override {
        vm.startPrank(callerWithETH);
        uint256 issuedAmount = trancheRouter.issue{value: 1 ether}(address(adapters[0]), maturity, 1 ether, receiver, 0);
    ...
    function testRedeemWithYT_RevertIfInsufficientAllowance() public override {
        ...
        data[1] = abi.encodeWithSignature("unwrapWETH9(uint256,address)", 0, receiver);
        Multicallable(address(trancheRouter)).multicall(data);
        vm.stopPrank();
    }

...
contract ForkTest is Base {
    ...
    function testFork_RemoveLiquidity(
        ...
        vm.assume(params_removeLiq.recipient != address(tricrypto)); // skip if recipient is tricrypto
    ...
    function addLiquidity(
        ...
        data[1] = abi.encodeCall(PeripheryPayments.sweepToken, (address(tricrypto), 0, sender));
        uint256 value;
        if (useEth) {
            // make sure recipient can be payable
            value = underlyingIn; // set value to `underlyingIn` if useEth
            data[2] = abi.encodeCall(PeripheryPayments.refundETH, ());
        } else {
            underlying.approve(address(router), underlyingIn);
            data[2] = abi.encodeCall(PeripheryPayments.sweepToken, (address(weth), 0, sender));
        }
        for (uint256 i = 0; i < 3; i++) {
            pts[i].approve(address(router), ptsIn[i]);
        }
        bytes[] memory ret = Multicallable(address(router)).multicall{value: value}(data);
        ...
        returns (uint256 underlyingOut, uint256[3] memory ptsOut)
    {
        ...
        if (useEth) {
            ...
            data[1] = abi.encodeCall(PeripheryPayments.sweepTokens, (tokens, new uint256[](3), recipient));
            data[2] = abi.encodeCall(PeripheryPayments.unwrapWETH9, (0, recipient));
            // batch call
            bytes[] memory ret = Multicallable(address(router)).multicall(data);
        ...
        returns (uint256 underlyingOut)
    {
        ...
        if (useEth) {
            ...
            // make sure recipient can be payable
            data[1] = abi.encodeCall(PeripheryPayments.unwrapWETH9, (0, recipient));
            bytes[] memory ret = Multicallable(address(router)).multicall(data);
        ...
        returns (uint256 underlyingIn)
    {
        ...
        if (useEth) {
            ...
            data[1] = abi.encodeCall(PeripheryPayments.refundETH, ());
            // In most cases, `ptDesired` eth would be enough to swap `ptDesired` principal token.
            bytes[] memory ret = Multicallable(address(router)).multicall{value: maxIn}(data);

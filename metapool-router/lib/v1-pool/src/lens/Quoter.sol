...
contract Quoter is IQuoter, INapierSwapCallback, INapierMintCallback {
    ...
    function swapCallback(int256 underlyingDelta, int256 ptDelta, bytes calldata data) external view override {
        ...
        if (_type == CallbackType.SwapYtForUnderlying) {
            ...
            // Revert containing the quote
            uint256 netUnderlying = underlyingRedeemed - (-underlyingDelta).toUint256();
            _revertWithQuoteData(netUnderlying);
        } else if (_type == CallbackType.SwapUnderlyingForYt) {
            ApproxParams memory approx = defaultApprox();
            approx.eps = 0.05 * 1e18; // 5%
            uint256 uDeposit = TrancheMathHelper.getApproxUnderlyingNeededByYt(
                ITranche(assets.principalTokens[index]), ytDesired, approx
            );

            if (uDeposit <= underlyingDelta.toUint256()) {
                _revertWithQuoteData(uDeposit); // netUnderlying is zero in this branch
            }
            // Subtract the amount of underlying we got from the swap
            uint256 netUnderlying = uDeposit - underlyingDelta.toUint256();
            // HACK: Pack `netUnderlying` and `uDeposit` into a single uint256
            // because the intentional revert data is limited to 32 bytes.
            // Revert containing the quote
            uint256 packedValue = (netUnderlying << 128) | uDeposit.toUint128();
            _revertWithQuoteData(packedValue);
        }
    }
    ...
    function quoteAddLiquidityOneUnderlyingKeepYt(INapierPool pool, uint256 index, uint256 underlyingIn)
        public
        view
        returns (uint256 liquidity, uint256 underlyingDeposit)
    {
        underlyingDeposit = approxUnderlyingToAddLiquidityOneUnderlyingKeepYt(pool, index, underlyingIn);
        // B: Reserve of base pool LP token before adding liquidity
        // ∆b: Base pool LP token amount to be swapped out
        // L: total Liquidity token amount before adding liquidity
        // s: shares of minted liquidity
        ...
        uint256 totalLiquidity = IERC20Metadata(address(pool)).totalSupply();
        liquidity = totalLiquidity * underlyingDeposit / totalUnderlying;
    }
        ...
        returns (uint256 liquidity, uint256 baseLptSwap)
    {
        ...
        (int256 underlyingAmount,,) = PoolMath.calculateSwap(
            state, PoolMath.computeAmmParameters(state), -(baseLptSwap * N_COINS * state.virtualPrice / WAD).toInt256()
        );
        ...
        returns (uint256 underlyingOut, uint256 gasEstimate)
    {
        ...
        if (block.timestamp < INapierPool(pool).maturity()) {
            ...
            state.totalBaseLptTimesN -= (baseLptWithdrawn * N_COINS * state.virtualPrice / WAD);
    ...
    function quoteUnderlyingForYt(INapierPool pool, uint256 index, uint256 ytOut)
        public
        returns (uint256 underlyingIn, uint256 uDeposit, uint256 gasEstimate)
    {
        uint256 _before = gasleft();
        ...
        try INapierPool(pool).swapPtForUnderlying(
            ...
            uint256 packed = _handleRevert(ret); // Note: `packed` contains two values in a single uint256
            (underlyingIn, uDeposit) = (packed >> 128, packed & type(uint128).max);
            gasEstimate = _before - gasleft();
        }
    }
    ...
    function approxUnderlyingToAddLiquidityOneUnderlyingKeepYt(
        INapierPool pool,
        uint256 index,
        ...
        ApproxParams memory approx = defaultApprox();
        approx.eps /= 10;
        approx.guessMax = underlyingsToAdd;
        bytes memory args = abi.encode(
            pool.tricrypto(), index, assets.principalTokens[index], pool.totalUnderlying(), pool.totalBaseLpt()
        );
        return LibApproximation.bisect(args, computeRelErrorForZapDeposit, underlyingsToAdd, approx);
    }

    function computeRelErrorForZapDeposit(uint256 midpoint, bytes memory args, uint256 underlyingsToAdd)
        private
        view
        returns (int256)
    {
        (
            ...
            uint256 totalBaseLpt
        ) = abi.decode(args, (CurveTricryptoOptimizedWETH, uint256, ITranche, uint256, uint256));

        // underlying amount -> pt amount
        uint256 ptIssued = _previewIssue(pt, midpoint);
        // pt amount -> baseLpt amount
        uint256[3] memory amounts;
        amounts[index] = ptIssued;
        uint256 baseLptIn = tricrypto.calc_token_amount(amounts, true);
        uint256 underlyingIn = underlyingsToAdd - midpoint;
        // Formula:
        // u: total underlying amount received from the user
        // ∆u: underlying token amount used for issuing the principal token
        // ∆b: baseLpt amount to be deposited to the NapierPool
        // U: total underlying reserve before adding liquidity
        // B: total baseLpt reserve before adding liquidity
        // The amount of underlying token deposited should be proportional to the reserve:
        // ```
        // (u - ∆u) : ∆b ~= U : B
        // ```
        //
        // We want to find ∆u such that minimize the epsilon:
        // ```
        // epsilon = (v - v_approx) / v = 1 - v_approx / v
        // v_approx = ∆b * U /(u - ∆u) * B
        // v = 1
        // ```
        return 1e18 - (totalUnderlying * baseLptIn * 1e18 / (totalBaseLpt * underlyingIn)).toInt256();
    }
    ...
    function approxBaseLptToRemoveLiquidityOnePt(INapierPool pool, uint256 liquidity) public view returns (uint256) {
        ...
        state.totalBaseLptTimesN -= ((baseLptOut * N_COINS * state.virtualPrice) / WAD);
    ...
    function _previewIssue(ITranche pt, uint256 underlyingAmount) internal view returns (uint256) {
        ...
        uint256 shares = underlyingAmount * WAD / cscale;
        uint256 fee = (shares * series.issuanceFee + MAX_BPS - 1) / MAX_BPS; // round up
        return (shares - fee) * series.maxscale / WAD;
    }
        ...
        returns (uint256 underlyingOut18, uint256 baseLptOut)
    {
        ...
        baseLptOut = (liquidity * state.totalBaseLptTimesN * WAD / (N_COINS * state.virtualPrice)) / totalLp;
    }

...
library Cast {
    function into(address x) internal pure returns (RsETHAdapter) {
        return RsETHAdapter(payable(address(x)));
    }
}

contract TestRsETHAdapter is BaseTestLSTAdapter {
    using Cast for *;
    ...
    function testProxyInitializer_RevertAfter_Initialize_Called() public {
        ...
        RsETHAdapter(payable(address(adapter))).initialize(owner, rebalancer, 0, 0);
    }
    ...
    function testImplementationInitializer_Revert() public {
        ...
        vm.expectRevert("Initializable: contract is already initialized");
        RsETHAdapter(payable(implementation)).initialize(attacker, rebalancer, 0, 0);
    }
    ...
    function testPrefundedDeposit() public override {
        ...
        assertApproxEqAbs(
            (balance * RSETH_ORACLE.rsETHPrice()) / 1e18,

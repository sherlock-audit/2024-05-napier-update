...
library Cast {
    function into(address x) internal pure returns (RswETHAdapter) {
        return RswETHAdapter(payable(address(x)));
    }
}

contract TestRswETHAdapter is BaseTestLSTAdapter {
    using Cast for *;
    ...
    function setUp() public override {
        ...
        vm.label(Constants.RSWETH, "RSWETH");
    }
    ...
    function testProxyInitializer_RevertAfter_Initialize_Called() public {
        ...
        RswETHAdapter(payable(address(adapter))).initialize(owner, rebalancer, 0, 0);
    }
    ...
    function testImplementationInitializer_Revert() public {
        ...
        vm.expectRevert("Initializable: contract is already initialized");
        RswETHAdapter(payable(implementation)).initialize(attacker, rebalancer, 0, 0);
    }
    ...
    function testPrefundedDeposit() public override {
        ...
        assertApproxEqAbs(
            (balance * RSWETH.rswETHToETHRate()) / 1e18,
